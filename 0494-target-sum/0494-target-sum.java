class Solution {
    public int findTargetSumWays(int[] nums, int target) {
        ...
        // 3. The 1D DP Array (Defaults to all 0s)
        int[] dp = new int[tar + 1];
        
        // Base Case: 1 way to make a sum of 0 (pick nothing)
        dp[0] = 1;

        // 4. The Loops (Outer: Items, Inner: Target backwards)
        for (int num : nums) {
            for (int j = tar; j >= num; j--) {
                // dp[j] (leave) + dp[j - num] (take)
                dp[j] = dp[j] + dp[j - num]; 
            }
        }
